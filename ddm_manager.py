...
class DDMApp(ctk.CTk):
    ...
    def _build_col_preview_log(self, parent):
        ...
        # Preview — Visualizador do documento
        prev_outer = ctk.CTkFrame(col, fg_color=BG_APP, corner_radius=0)
        prev_outer.grid(row=0, column=0, sticky="nsew", padx=0, pady=0)
        prev_outer.grid_rowconfigure(1, weight=1)
        prev_outer.grid_columnconfigure(0, weight=1)

        prev_hdr = ctk.CTkFrame(prev_outer, fg_color=BG_CARD,
                                corner_radius=0, height=32)
        prev_hdr.grid(row=0, column=0, sticky="ew")
        prev_hdr.grid_propagate(False)
        prev_hdr.grid_columnconfigure(0, weight=1)
        ctk.CTkLabel(prev_hdr, text="VISUALIZADOR DO DOCUMENTO",
                     font=FONT_LABEL, text_color=TXT_DIM
                     ).grid(row=0, column=0, padx=12, pady=4, sticky="w")
        ctk.CTkLabel(prev_hdr, text="(prévia do preenchimento)",
                     font=("Segoe UI",8), text_color=TXT_DIM
                     ).grid(row=0, column=1, padx=8, pady=4, sticky="e")

        # Área de scroll do documento
        self._prev_scroll = ctk.CTkScrollableFrame(
            prev_outer, fg_color="#FFFFFF", corner_radius=0,
            scrollbar_button_color=BORDER,
            scrollbar_button_hover_color=ACCENT)
        self._prev_scroll.grid(row=1, column=0, sticky="nsew")
        self._prev_scroll.grid_columnconfigure(0, weight=1)

        # Placeholder inicial
        self._prev_placeholder = ctk.CTkLabel(
            self._prev_scroll,
            text="Selecione um DDM na lista\npara visualizar o documento",
            font=("Segoe UI", 11), text_color="#AAAAAA",
            justify="center")
        self._prev_placeholder.grid(row=0, column=0, pady=60)

        # Log
        log_hdr = ctk.CTkFrame(col, fg_color=BG_CARD, corner_radius=0, height=32)
        log_hdr.grid(row=1, column=0, sticky="sew")
        ...
        col.grid_rowconfigure(1, minsize=160)
    ...
    def _build_footer(self):
        ...
        ctk.CTkButton(footer, text="📊   Relatório",
                      ...
                      ).grid(row=0, column=3, padx=6, sticky="w", **py)
        ctk.CTkButton(footer, text="✖   Sair",
                      font=FONT_BTN, fg_color=BG_CARD, text_color=ERROR,
                      hover_color=BORDER,
                      command=self._sair
                      ).grid(row=0, column=4, padx=(6,12), sticky="e", **py)
    ...
    def _atualizar_preview(self, item: dict):
        """Renderiza visualizador formatado do DDM no painel de preview."""
        # Limpa conteúdo anterior
        for w in self._prev_scroll.winfo_children():
            w.destroy()

        if not item or not item.get("docx"):
            self._prev_placeholder = ctk.CTkLabel(
                self._prev_scroll,
                text="Selecione um DDM na lista\npara visualizar o documento",
                font=("Segoe UI", 11), text_color="#AAAAAA", justify="center")
            self._prev_placeholder.grid(row=0, column=0, pady=60)
            return

        # Monta dados do cabeçalho
        data_ddm     = item.get("data_ddm") or datetime.date.today().strftime("%d/%m/%Y")
        setor        = self._v_setor.get()
        subarea      = self._v_subarea.get()
        turno        = self._v_turno.get()
        facilitador  = self._v_facilitador.get()
        ...
        # Lê OBJETIVO do docx real (se disponível)
        objetivo = ""
        try:
            ...
            for t in doc.tables:
                for row in t.rows:
                    for cell in row.cells:
                        txt = cell.text.strip()
                        if txt.startswith("OBJETIVO:") or txt.startswith("OBJETIVO "):
                            objetivo = txt.split(":", 1)[-1].strip()
                            break
        except: pass

        # ── Papel A4 simulado ──
        papel = ctk.CTkFrame(self._prev_scroll, fg_color="#FFFFFF",
                             corner_radius=4)
        papel.grid(row=0, column=0, sticky="ew", padx=8, pady=8)
        papel.grid_columnconfigure(0, weight=1)

        row_idx = 0

        def add_sep():
            nonlocal row_idx
            ctk.CTkFrame(papel, fg_color="#DDDDDD", height=1, corner_radius=0
                         ).grid(row=row_idx, column=0, sticky="ew", padx=0)
            row_idx += 1

        def add_field(label: str, valor: str, label_color="#003F7F",
                      valor_color="#1A1A1A", bg="#FFFFFF", bold_val=False):
            nonlocal row_idx
            f = ctk.CTkFrame(papel, fg_color=bg, corner_radius=0)
            f.grid(row=row_idx, column=0, sticky="ew", padx=0)
            f.grid_columnconfigure(1, weight=1)
            ctk.CTkLabel(f, text=label,
                         font=("Segoe UI", 9, "bold"),
                         text_color=label_color, anchor="w", width=110
                         ).grid(row=0, column=0, padx=(8,4), pady=4, sticky="w")
            ctk.CTkLabel(f, text=valor,
                         font=("Segoe UI", 9, "bold" if bold_val else "normal"),
                         text_color=valor_color, anchor="w", wraplength=230
                         ).grid(row=0, column=1, padx=(0,8), pady=4, sticky="ew")
            row_idx += 1

        def add_title():
            nonlocal row_idx
            f = ctk.CTkFrame(papel, fg_color="#003F7F", corner_radius=0)
            f.grid(row=row_idx, column=0, sticky="ew")
            f.grid_columnconfigure(0, weight=1)
            ctk.CTkLabel(f, text="❄  DIÁLOGO DIÁRIO METALFRIO",
                         font=("Segoe UI", 11, "bold"),
                         text_color="#FFFFFF"
                         ).grid(row=0, column=0, pady=8, padx=12, sticky="w")
            ctk.CTkLabel(f, text="DDM",
                         font=("Segoe UI", 10, "bold"),
                         text_color="#00D4FF"
                         ).grid(row=0, column=1, pady=8, padx=12, sticky="e")
            row_idx += 1
        ...
        # Linha 1: Setor | Data | Turno
        f1 = ctk.CTkFrame(papel, fg_color="#EBF4FF", corner_radius=0)
        f1.grid(row=row_idx, column=0, sticky="ew")
        ...
        for col_i, (lbl, val) in enumerate([
            ...
            ctk.CTkLabel(f1, text=lbl,
                         font=("Segoe UI",8,"bold"), text_color="#003F7F"
                         ).grid(row=0, column=col_i, padx=6, pady=(4,0), sticky="w")
            ctk.CTkLabel(f1, text=val,
                         font=("Segoe UI",9), text_color="#1A1A1A"
                         ).grid(row=1, column=col_i, padx=6, pady=(0,4), sticky="w")
        row_idx += 1
        add_sep()

        # Subárea | Carga
        f2 = ctk.CTkFrame(papel, fg_color="#FFFFFF", corner_radius=0)
        f2.grid(row=row_idx, column=0, sticky="ew")
        f2.grid_columnconfigure((0,1), weight=1)
        ctk.CTkLabel(f2, text="SUBÁREA:", font=("Segoe UI",8,"bold"),
                     text_color="#003F7F").grid(row=0,column=0,padx=6,pady=(4,0),sticky="w")
        ctk.CTkLabel(f2, text=subarea, font=("Segoe UI",9),
                     text_color="#1A1A1A").grid(row=1,column=0,padx=6,pady=(0,4),sticky="w")
        ctk.CTkLabel(f2, text="CARGA HORÁRIA:", font=("Segoe UI",8,"bold"),
                     text_color="#003F7F").grid(row=0,column=1,padx=6,pady=(4,0),sticky="w")
        ctk.CTkLabel(f2, text="10 MIN", font=("Segoe UI",9),
                     text_color="#1A1A1A").grid(row=1,column=1,padx=6,pady=(0,4),sticky="w")
        row_idx += 1
        add_sep()

        add_field("FACILITADOR:", facilitador, bg="#EBF4FF")
        add_sep()
        add_field("TEMA:", tema, bold_val=True)
        add_sep()
        if objetivo:
            add_field("OBJETIVO:", objetivo, bg="#F8FAFC")
            add_sep()

        # Cabeçalho tabela participantes
        th = ctk.CTkFrame(papel, fg_color="#003F7F", corner_radius=0)
        th.grid(row=row_idx, column=0, sticky="ew")
        th.grid_columnconfigure(1, weight=1)
        for ci, (t, w) in enumerate([("Nº",28),("NOME",0),("RE",50),("ASSINATURA",70)]):
            ctk.CTkLabel(th, text=t, font=("Segoe UI",8,"bold"),
                         text_color="#FFFFFF", width=w, anchor="center"
                         ).grid(row=0, column=ci, padx=4, pady=3)
        row_idx += 1

        # Participantes
        for pi, (re_v, nome_v) in enumerate(PARTICIPANTES):
            bg_p = "#F8FAFC" if pi % 2 == 0 else "#FFFFFF"
            pr = ctk.CTkFrame(papel, fg_color=bg_p, corner_radius=0)
            pr.grid(row=row_idx, column=0, sticky="ew")
            pr.grid_columnconfigure(1, weight=1)
            ctk.CTkLabel(pr, text=str(pi+1), font=("Segoe UI",8),
                         text_color="#666666", width=28, anchor="center"
                         ).grid(row=0, column=0, padx=4, pady=2)
            ctk.CTkLabel(pr, text=nome_v, font=("Segoe UI",8),
                         text_color="#1A1A1A", anchor="w"
                         ).grid(row=0, column=1, padx=4, pady=2, sticky="ew")
            ctk.CTkLabel(pr, text=re_v, font=("Segoe UI",8),
                         text_color="#444444", width=50, anchor="center"
                         ).grid(row=0, column=2, padx=4, pady=2)
            ctk.CTkLabel(pr, text="", width=70
        ...
        def worker():
            ...
            if len(pdfs) == 1:
                ...
                self.after(0, lambda: self._log(
                    f"   Combinando {len(pdfs)} PDFs...", TXT_SEC))
                try:
                    ...
                    pdf_final = str(Path(pasta) / "DDM_SEMANA_COMBINADO.pdf")
                    with open(pdf_final, "wb") as f_out:
                        writer.write(f_out)
                    self.after(0, lambda: self._log(
                        f"✔  PDF combinado: {Path(pdf_final).name}", SUCCESS))
                    registrar_log("SEMANA", pdf_final, "PDF_COMBINADO", 0, True)
