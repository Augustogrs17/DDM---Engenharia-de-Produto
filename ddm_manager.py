...
class MainWindow(tk.Tk):
    ...
    def _build(self):
        ...
        # ── Barra de pasta raiz (largura total, abaixo do header) ──
        raiz_bar = tk.Frame(self, bg=CINZA1, pady=6)
        raiz_bar.pack(fill="x", padx=14)

        tk.Label(raiz_bar, text="📁  Pasta raiz DDM 2026", font=F_LBL,
                 bg=CINZA1, fg=VERDE).pack(side="left", padx=(8,6))
        ...
        tk.Button(raiz_bar, text="  …  ", font=("Segoe UI",10,"bold"),
                  bg=VERDE, fg=AZUL, relief="flat", cursor="hand2",
                  command=self._browse).pack(side="left", padx=(4,8), ipady=5)

        tk.Button(raiz_bar, text="🔄  Atualizar", font=F_BTN_SM,
                  bg=CINZA2, fg=VERDE, relief="flat", cursor="hand2",
                  command=self._varrer).pack(side="left", padx=(0,8), ipady=5, ipadx=8)
        ...
        esq = tk.Frame(body, bg=AZUL, width=310)
        ...
        # ── Rodapé ──
        rod = tk.Frame(self, bg=CINZA1, height=68)
        rod.pack(fill="x", side="bottom"); rod.pack_propagate(False)

        tk.Button(rod,
                  text="📄   Gerar DOCX selecionados",
                  font=("Segoe UI", 12, "bold"),
                  bg=CINZA2, fg=VERDE, relief="flat", cursor="hand2",
                  padx=18, command=self._gerar_docx_sel
                  ).pack(side="left", padx=10, pady=12, ipady=4)

        tk.Button(rod,
                  text="📑   Gerar PDF + Abrir selecionados",
                  font=("Segoe UI", 12, "bold"),
                  bg=VERDE, fg=AZUL, relief="flat", cursor="hand2",
                  padx=18, command=self._gerar_pdf_sel
                  ).pack(side="left", padx=4, pady=12, ipady=4)
        ...
        tk.Button(rod,
                  ...
                  ).pack(side="right", padx=10, pady=12, ipady=4)
