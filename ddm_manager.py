...
class MainWindow(tk.Tk):
    ...
    def _mesclar_docxs(self, docxs: list, pasta_saida: Path) -> Path:
        """
        Combina vários DOCX num único arquivo usando o Word via win32com.
        Cada documento é inserido com SubDocument/Insert para preservar formatação.
        """
        try:
            import win32com.client
        except ImportError:
        ...
        saida = (pasta_saida / "DDM_SEMANA_COMBINADO.docx").resolve()
        word  = None
        try:
            word = win32com.client.Dispatch("Word.Application")
            word.Visible = False
            ...
            for docx_path in docxs[1:]:
                ...
                rng.InsertBreak(7)  # 7 = wdPageBreak

                # Move para o final novamente e insere o próximo arquivo
                rng = doc_base.Range()
                rng.Collapse(0)
                rng.InsertFile(str(docx_path.resolve()))

            doc_base.SaveAs(str(saida))
            doc_base.Close(False)
        finally:
            if word:
                try: word.Quit()
                except: pass
