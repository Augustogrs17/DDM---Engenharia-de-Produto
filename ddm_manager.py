...
def converter_pdfs_word(caminhos: list) -> list:
    """
    Converte uma lista de DOCX → PDF usando uma única instância do Word.
    Retorna lista de (Path_pdf | None, erro | "") para cada entrada.
    """
    try:
    ...
    try:
        ...
        for caminho_docx in caminhos:
            ...
            try:
                ...
                doc.SaveAs(str(caminho_pdf), FileFormat=17)  # wdFormatPDF
...
class LoginWindow(tk.Tk):
    ...
    def _build(self):
        ...
        frm.pack(fill="both", expand=True, pady=(16, 24))
        ...
        tk.Button(frm, text="Entrar →", font=F_BTN,
                  ...
                  command=self._login).pack(fill="x", pady=(4,0), ipady=10)
...
class MainWindow(tk.Tk):
    ...
    def _build(self):
        ...
        # ══ 3b. BARRA PASTA DE SAÍDA ═════════════════════════════════════════
        saida_bar = tk.Frame(self, bg=AZUL, pady=0)
        saida_bar.pack(fill="x", padx=14, pady=(4,0))

        tk.Label(saida_bar, text="💾  Salvar em", font=F_LBL,
                 bg=AZUL, fg=CINZA3).pack(side="left", padx=(0,6))

        self._v_saida = tk.StringVar(value="")
        self._lbl_saida = tk.Label(saida_bar,
                 textvariable=self._v_saida, font=F_FIEL,
                 bg=AZUL, fg=TEXTO, anchor="w")
        self._lbl_saida.pack(side="left", fill="x", expand=True)

        tk.Label(saida_bar, text="(padrão: mesma pasta do DDM)", font=F_SMALL,
                 bg=AZUL, fg=CINZA3).pack(side="left", padx=(4,8))

        tk.Button(saida_bar, text="Alterar", font=F_BTN_SM,
                  bg=CINZA2, fg=VERDE, relief="flat", cursor="hand2",
                  command=self._browse_saida
                  ).pack(side="left", padx=(0,4), ipady=4, ipadx=8)

        tk.Button(saida_bar, text="✕", font=F_BTN_SM,
                  bg=CINZA2, fg=ERR, relief="flat", cursor="hand2",
                  command=lambda: self._v_saida.set("")
    ...
    def _browse_saida(self):
        p = filedialog.askdirectory(title="Escolha onde salvar os arquivos gerados")
        if p:
            self._v_saida.set(p)
    ...
    # ── Gerar DOCX ───────────────────────────────────────────────────────
    def _pasta_saida(self, fallback: Path) -> Path:
        """Retorna pasta de saída escolhida ou mesma pasta do DDM."""
    ...
    def _gerar_pdf_sel(self):
        ...
        # 1. Gera todos os DOCX preenchidos
        docxs_para_converter = []
        for d in sel:
            src   = d["docx_path"]
            pasta = self._pasta_saida(src.parent)
            ...
            try:
                ...
                self._log(f"✔ {d['dia_nome']}: {info['nomes']} nomes preenchidos")
                docxs_para_converter.append((d["dia_nome"], docx))
            except Exception as e:
                self._log(f"✖ {d['dia_nome']} (DOCX): {e}", ERR)

        if not docxs_para_converter:
            return

        # 2. Converte todos com uma única instância do Word
        self._log(f"\n  Abrindo Word para converter {len(docxs_para_converter)} arquivo(s)...")
        try:
            resultados = converter_pdfs_word([p for _, p in docxs_para_converter])
        except Exception as e:
            self._log(f"✖ Erro ao iniciar Word: {e}", ERR)
            return

        pdfs_gerados = []
        for (dia_nome, _), (pdf_path, erro) in zip(docxs_para_converter, resultados):
            if pdf_path:
                self._log(f"✔ {dia_nome}: {pdf_path.name}", OK_C)
                pdfs_gerados.append(pdf_path)
            else:
                self._log(f"✖ {dia_nome}: {erro}", ERR)

        if not pdfs_gerados:
            return

        # 3. Abre: 1 PDF direto, múltiplos → mescla
        if len(pdfs_gerados) == 1:
            os.startfile(str(pdfs_gerados[0]))
        else:
            self._log(f"\n  Mesclando {len(pdfs_gerados)} PDFs...")
            try:
                # PDF combinado vai para pasta de saída escolhida
                pasta_merged = self._pasta_saida(pdfs_gerados[0].parent)
                merged = self._mesclar_pdfs(pdfs_gerados, pasta_merged)
    ...
    def _mesclar_pdfs(self, pdfs: list, pasta_saida: Path = None) -> Path:
        """Mescla lista de PDFs em um único arquivo."""
        try:
        ...
        pasta = pasta_saida or pdfs[0].parent
        saida = pasta / "DDM_SEMANA_COMBINADO.pdf"
