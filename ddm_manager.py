...
class MainWindow(tk.Tk):
    ...
    def _build(self):
        ...
        esq = tk.Frame(body, bg=AZUL, width=320)
        ...
        def campo(key, label, tem_browse=False):
            f = tk.Frame(esq, bg=AZUL); f.pack(fill="x", pady=2)
            tk.Label(f, text=label, font=F_LBL, bg=AZUL,
                     fg=VERDE, anchor="w").pack(fill="x")
            row = tk.Frame(f, bg=AZUL); row.pack(fill="x")
            v = tk.StringVar(value=DEFAULTS.get(key, ""))
            self._vars[key] = v
            e = tk.Entry(row, textvariable=v, font=F_FIEL, width=30,
                         bg=CINZA2, fg=TEXTO, insertbackground=VERDE,
                         relief="flat", bd=0)
        ...
        campo("raiz",        "📁  Pasta raiz DDM 2026", tem_browse=True)
        ...
        tk.Button(esq, text="🔄  Atualizar semana", font=F_BTN_SM,
                  bg=CINZA2, fg=VERDE, relief="flat", cursor="hand2",
                  command=self._varrer).pack(fill="x", pady=(10,0), ipady=5)

        # Coluna central: lista DDMs da semana
        meio = tk.Frame(body, bg=AZUL, width=330)
        meio.pack(side="left", fill="y", padx=(0,10))
        meio.pack_propagate(False)

        seg, sex = semana_atual(datetime.date.today())
        tk.Label(meio,
                 text=f"DDMs da semana  {seg:%d/%m} – {sex:%d/%m}",
        ...
        tk.Label(hdr_list, text="Dia", font=F_SMALL, bg=CINZA1,
                 fg=CINZA3, width=10, anchor="w").pack(side="left")
        ...
        tk.Button(rod, text="📄  Gerar DOCX selecionados", font=F_BTN,
                  bg=CINZA2, fg=VERDE, relief="flat", cursor="hand2",
                  padx=12, command=self._gerar_docx_sel).pack(side="left", padx=8, pady=10)
        tk.Button(rod, text="🖨️  Gerar PDF + Abrir selecionados", font=F_BTN,
                  bg=VERDE, fg=AZUL, relief="flat", cursor="hand2",
                  padx=12, command=self._gerar_pdf_sel).pack(side="left", padx=4, pady=10)
    ...
    def _rebuild_lista(self):
        ...
        for d in self._semana:
            ...
            row = tk.Frame(self._lista_frm, bg=bg, pady=2)
            row.pack(fill="x", pady=1)

            cb = tk.Checkbutton(row, variable=v, bg=bg,
                                activebackground=bg,
                                selectcolor=CINZA1,
                                state="normal" if tem_ddm else "disabled")
            cb.pack(side="left", padx=6)

            tk.Label(row, text=d["dia_nome"], font=F_FIEL,
                     bg=bg, fg=fg_dia, width=11, anchor="w").pack(side="left")

            if tem_ddm:
                tema_txt = d["tema"][:28] if len(d["tema"]) > 28 else d["tema"]
                tk.Label(row, text=tema_txt, font=F_SMALL,
                         bg=bg, fg=TEXTO, anchor="w").pack(side="left", padx=4)
                tk.Label(row, text=d["data_ddm"], font=F_SMALL,
                         bg=bg, fg=CINZA3).pack(side="right", padx=6)
            else:
    ...
    def _gerar_docx_sel(self):
        ...
        for d in sel:
            ...
            try:
                ...
                self._log(f"✔ {d['dia_nome']}: {dst.name} "
                          f"({info['nomes']} nomes)", OK_C)
                gerados.append(dst)
            except Exception as e:
                self._log(f"✖ {d['dia_nome']}: {e}", ERR)
        if gerados and messagebox.askyesno("Abrir?",
                f"{len(gerados)} arquivo(s) gerado(s).\nAbrir pasta?"):
    ...
    def _gerar_pdf_sel(self):
        ...
        for d in sel:
            ...
            try:
                ...
                self._log(f"✔ {d['dia_nome']}: DOCX preenchido "
                          f"({info['nomes']} nomes)")
                self._log(f"  Convertendo para PDF...")
                pdf = converter_pdf_word(docx)
                self._log(f"✔ PDF: {pdf.name}", OK_C)
